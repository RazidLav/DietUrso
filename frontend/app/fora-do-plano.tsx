...
export default function OffPlanScreen() {
  ...
  const filteredFoods = foods.filter((food) => matchesSearch(query, food.name, food.brand, food.category));
  const filteredRecipes = recipes.filter((recipe) => matchesSearch(query, recipe.name, recipe.category, recipe.description));
      ...
      <Modal visible={picker} transparent animationType="slide" onRequestClose={() => setPicker(false)} statusBarTranslucent>
        <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={styles.modal} accessibilityViewIsModal>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>Adicionar item</Text><Pressable accessibilityRole="button" accessibilityLabel="Fechar seletor" onPress={() => setPicker(false)}><MaterialDesignIcons name="close" size={24} color={colors.onSurface} /></Pressable></View>
            <View style={styles.pickerTabs}><Pressable onPress={() => setPickerType("food")} style={[styles.chip, pickerType === "food" && styles.chipOn]}><Text style={[styles.chipText, pickerType === "food" && styles.chipTextOn]}>Alimentos</Text></Pressable><Pressable onPress={() => setPickerType("recipe")} style={[styles.chip, pickerType === "recipe" && styles.chipOn]}><Text style={[styles.chipText, pickerType === "recipe" && styles.chipTextOn]}>Receitas</Text></Pressable></View>
            <View style={styles.searchWrap}><MaterialDesignIcons name="magnify" size={19} color={colors.onSurfaceTertiary} /><TextInput value={query} onChangeText={setQuery} placeholder="Pesquisar nome ou marca" placeholderTextColor={colors.onSurfaceTertiary} style={styles.search} accessibilityLabel="Pesquisar itens" />{query ? <Pressable accessibilityLabel="Limpar pesquisa" onPress={() => setQuery("")}><MaterialDesignIcons name="close-circle" size={20} color={colors.onSurfaceTertiary} /></Pressable> : null}</View>
            <ScrollView style={styles.pickerList} contentContainerStyle={{ gap: spacing.sm, paddingBottom: spacing.xl }} keyboardShouldPersistTaps="handled" nestedScrollEnabled>{pickerType === "food" ? filteredFoods.map((food) => <Pressable key={food.id} style={styles.pick} onPress={() => addFood(food)}><View style={{ flex: 1 }}><Text style={styles.itemName}>{food.name}</Text><Text style={styles.muted}>{food.brand ? `${food.brand} · ` : ""}{food.referenceQuantity} {food.referenceUnit} · {Math.round(food.nutrients.kcal)} kcal</Text></View><MaterialDesignIcons name="plus-circle" size={23} color={colors.brandPrimary} /></Pressable>) : filteredRecipes.map((recipe) => { const nutrition = recipeNutrients(recipe); return <Pressable key={recipe.id} style={styles.pick} onPress={() => addRecipe(recipe)}><View style={{ flex: 1 }}><Text style={styles.itemName}>{recipe.name}</Text><Text style={styles.muted}>1 porção · {Math.round(nutrition.perServing.kcal)} kcal</Text></View><MaterialDesignIcons name="plus-circle" size={23} color={colors.brandPrimary} /></Pressable>; })}{pickerType === "food" && !filteredFoods.length ? <View style={styles.empty}><Text style={styles.emptyTitle}>Nenhum alimento encontrado</Text><Text style={styles.muted}>Tente outro nome, marca ou categoria.</Text></View> : null}{pickerType === "recipe" && !filteredRecipes.length ? <View style={styles.empty}><Text style={styles.emptyTitle}>Nenhuma receita encontrada</Text></View> : null}</ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  kindness: { flexDirection: "row", alignItems: "center", gap: spacing.md, backgroundColor: withAlpha(colors.brandTertiary, "18"), borderWidth: 1, borderColor: withAlpha(colors.brandTertiary, "55"), borderRadius: radius.lg, padding: spacing.lg }, kindnessText: { flex: 1, color: colors.onSurfaceSecondary, fontSize: 12, lineHeight: 18 }, label: { color: colors.onSurfaceSecondary, fontSize: 11, fontWeight: "700", marginBottom: 6 }, input: { color: colors.onSurface, backgroundColor: colors.surfaceSecondary, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: spacing.md }, row: { flexDirection: "row", gap: spacing.sm },
  ...
  overlay: { flex: 1, backgroundColor: "#000A", justifyContent: "flex-end" }, modal: { height: "82%", maxHeight: 720, minHeight: 0, width: "100%", maxWidth: 720, alignSelf: "center", backgroundColor: colors.surfaceSecondary, borderTopLeftRadius: radius.lg, borderTopRightRadius: radius.lg, padding: spacing.lg, borderWidth: 1, borderColor: colors.border }, modalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, modalTitle: { color: colors.onSurface, fontSize: 18, fontWeight: "800" }, pickerTabs: { flexDirection: "row", gap: spacing.sm, marginVertical: spacing.md }, searchWrap: { flexDirection: "row", alignItems: "center", gap: spacing.sm, backgroundColor: colors.surfaceTertiary, borderRadius: radius.md, paddingHorizontal: spacing.md, marginBottom: spacing.md }, search: { flex: 1, color: colors.onSurface, paddingVertical: spacing.md }, pickerList: { flex: 1, minHeight: 0 }, pick: { flexDirection: "row", alignItems: "center", gap: spacing.sm, padding: spacing.md, backgroundColor: colors.surfaceTertiary, borderRadius: radius.md },
});
